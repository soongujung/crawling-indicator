...
if __name__ == '__main__':
    url = "http://ecos.bok.or.kr/api/StatisticSearch/{}/json/kr/1/50000/005Y003/MM/20140101/20191231/BECBLA02".format(
        api_key)
    ...
    # '202004' 같은 형태로 오는 데이터를 datetime.date 형태로 변환한 후에 넘겨주자.
    df_kospi_insert = df_kospi_insert.astype(dtype=dict_columns_type)

    df_kospi_insert.to_sql(name='loan_corporate_month',
                           con=alchemy_conn,
                           index=False,
                           index_label='TIME',
    ...
    df_kospi_select = pandas_sql.read_sql_query("select * from kospi_day", alchemy_conn)
